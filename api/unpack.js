// Vercel Serverless Function: POST /api/unpack
// Body esperado (JSON):
// { "packet": "000000000ce4786956c969eb534c1bdaae00232810" }

function isHexString(s) {
  return typeof s === "string" && s.length % 2 === 0 && /^[0-9a-fA-F]+$/.test(s);
}

function toUtcString(epochSeconds) {
  // Formato: "YYYY-MM-DD HH:MM:SS UTC"
  const d = new Date(epochSeconds * 1000);
  const pad = (n) => String(n).padStart(2, "0");
  return `${d.getUTCFullYear()}-${pad(d.getUTCMonth() + 1)}-${pad(d.getUTCDate())} ${pad(
    d.getUTCHours()
  )}:${pad(d.getUTCMinutes())}:${pad(d.getUTCSeconds())} UTC`;
}

function unpackPacket(hex) {
  if (!isHexString(hex)) {
    const err = new Error("Invalid hex string (must be even-length and only 0-9a-f).");
    err.statusCode = 400;
    throw err;
  }

  const buf = Buffer.from(hex, "hex");

  // Python:
  // FULL: "<IIiihbH" -> 4 + 4 + 4 + 4 + 2 + 1 + 2 = 21? Ojo:
  // En tu comentario dice 19 bytes. Esa discrepancia suele venir de que realmente es "<IIiihBH" o similar.
  // PERO tu ejemplo hex tiene 22 bytes? Vamos a seguir tu lógica real: tamaños calculados en Python.
  //
  // Igual en Node: vamos a decidir por longitud exacta como en tu código:
  // - FULL: 19 bytes
  // - ALERT: 9 bytes
  //
  // Si tus tamaños reales difieren, decime el largo real de cada mensaje y lo ajusto.
  const MESSAGE_SIZE_FULL = 21;
  const MESSAGE_SIZE_ALERT = 9;

  if (buf.length === MESSAGE_SIZE_FULL) {
    // Little-endian:
    // sequence_number: uint32
    // time: uint32
    // latitude: int32
    // longitude: int32
    // elevation: int16
    // temperature: int8
    // battery_voltage: uint16
    let o = 0;
    const sequence_number = buf.readUInt32LE(o); o += 4;
    const time = buf.readUInt32LE(o); o += 4;
    const latitude = buf.readInt32LE(o); o += 4;
    const longitude = buf.readInt32LE(o); o += 4;
    const elevation = buf.readInt16LE(o); o += 2;
    const temperature = buf.readInt8(o); o += 1;
    const battery_voltage = buf.readUInt16LE(o); o += 2;

    return {
      Type: "Periodic uplink",
      "Sequence Number": sequence_number,
      "Time (epoch s)": time,
      "Time (UTC)": toUtcString(time),
      Latitude: latitude * 1e-7,
      Longitude: longitude * 1e-7,
      "Altitude (meters)": elevation,
      "Temperature (degrees C)": temperature,
      "Battery Voltage (mV)": battery_voltage,
    };
  }

  if (buf.length === MESSAGE_SIZE_ALERT) {
    // "<II?" => uint32, uint32, bool(1 byte)
    let o = 0;
    const sequence_number = buf.readUInt32LE(o); o += 4;
    const time = buf.readUInt32LE(o); o += 4;
    const has_water = buf.readUInt8(o) !== 0; o += 1;

    return {
      Type: "Alert uplink",
      "Sequence Number": sequence_number,
      "Time (epoch s)": time,
      "Time (UTC)": toUtcString(time),
      "Has Water": has_water,
    };
  }

  const err = new Error(
    `Unknown packet size: ${buf.length} bytes (expected ${MESSAGE_SIZE_FULL} or ${MESSAGE_SIZE_ALERT})`
  );
  err.statusCode = 400;
  throw err;
}

export default async function handler(req, res) {
  try {
    if (req.method !== "POST") {
      res.setHeader("Allow", "POST");
      return res.status(405).json({ error: "Method Not Allowed. Use POST." });
    }

    // Vercel parsea JSON automáticamente si Content-Type: application/json
    const { packet } = req.body || {};
    if (!packet) {
      return res.status(400).json({ error: 'Missing "packet" in JSON body.' });
    }

    const result = unpackPacket(packet);
    return res.status(200).json(result);
  } catch (e) {
    const code = e.statusCode || 500;
    return res.status(code).json({ error: e.message || "Internal error" });
  }
}